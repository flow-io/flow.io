	...
	*/
	function onData( accessors, filters ) {
		var aKeys = Object.keys( accessors ),
			numAccessors = aKeys.length,
			fKeys = Object.keys( filters ),
			numFilters = fKeys.length,
			accessor, key, value, filter;
		...
		for ( var i = 0; i < numAccessors; i++ ) {
			...
			if ( !filters.hasOwnProperty( key ) ) {
				throw new Error( 'find()::unable to initialize stream. Accessor does not contain a corresponding filter: ' + key + '.' );
			}
		}
		/**
		* FUNCTION: onData( d )
