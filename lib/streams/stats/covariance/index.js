...
(function() {
	...
	*/
	Stream.prototype.matrix = function( arr ) {
		var cov = this._cov,
			matrix;
		if ( !arguments.length ) {
			matrix = new Array( cov.length );
			for ( var i = 0; i < cov.length; i++ ) {
				matrix[ i ] = new Array( cov[ i ].length );
	...
	*/
	Stream.prototype.means = function( arr ) {
		var means;
		if ( !arguments.length ) {
			means = new Array( this._means.length );
			for ( var i = 0; i < this._means.length; i++ ) {
				means[ i ] = this._means[ i ];
			}
			return means;
		}
		this._means = arr;
	...
	*/
	Stream.prototype.stream = function() {
		...
		// Create a transform stream to stringify the statistic:
